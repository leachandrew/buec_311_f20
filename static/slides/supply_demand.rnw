...
quantity demanded.
	...
	\item Recall that other factors also affect the quantity demanded.
		...
		\end{itemize}
	\item[]
	\item As an example, let's consider the effect of a ten thousand dollar increase in income per
capita.
	\end{itemize}
}
...
\frame{
	...
	\item How the demand curve shifts depends on the factor being considered.
		...
		\end{itemize}
	\item[]
	\item As another example, let's consider the effects of a 5 dollar increase in the price of a
bottle of wine.
	\end{itemize}
}
...
\caption{The effects of an increase in the price of wine on the demand for gasoline}
\end{figure}
}
...
\frame{
	...
	\item The demand function is given by:
		...
		Q = D(p,Y,X)
		\end{align*}
	where $Q$ is the quantity demanded, and $D(-)$ is the demand function that depends on the price,
